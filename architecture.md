...
Motion for React is the selected general-purpose runtime for coordinated entrances and exits, page transitions, scroll reveals, scroll-linked transforms, bounded parallax, and justified layout animation. Plan 010 introduced its first narrow footer use; Plan 011 establishes strict lazy feature loading, shared reduced-motion policy, and the first approved landing effect. CSS layout retains ownership of document flow. Touch, keyboard, upward, modal, and reduced-motion scrolling remain browser-owned. Ordinary downward desktop wheel distance is replayed 1:1 through one footer-owned animation-frame controller above the final viewport, then eased only within that viewport. This is a measured product exception rather than a general smooth-scroll system. The platform scrollbar remains visible with restrained CSS styling.
