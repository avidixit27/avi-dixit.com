...
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Entry and shell    | `src/main.tsx` initializes React; `src/app/App.tsx` composes the router, navigation, route-transition boundary, and footer                                                                                                                        |
| Routes             | `/`, `/shop`, and `/contact`, plus a reachable fallback route, declared in `src/app/RouteTransitionBoundary.tsx`                                                                                                                                  |
...
| Release visibility | `src/app/featureAvailability.ts` keeps Shop unreleased and Contact released; normal development and production omit Shop navigation and routing, while `npm run dev:all-features` enables every known feature only in the Vite development server |
...
The application shell and portfolio now have explicit feature-oriented ownership, cross-component DOM coordination uses an explicit element reference, and affected global effects have local cleanup paths. `RouteTransitionBoundary` owns secondary-route lazy readiness, the short accessible exit reveal, and navigation focus/scroll policy: push and replace reset to the top and focus the current route container; pop navigation preserves browser-managed scroll and focus. The primary Home route remains eager so its first hero can render beneath an outgoing route without an intermediate blank frame. `PortfolioScrollComposition` owns the home route's feature-local editorial copy, native sticky media story, bounded image drift, and locally scoped stacked sections; the route continues to compose it between the hero and photo grid. Automated regression coverage protects photo navigation, gallery timers and listeners, route links, contact-form presentation, shop cart behavior, route history/focus/scroll behavior, and core desktop/mobile gallery journeys. The responsive catalog supplies intrinsic dimensions and generated sources without exposing original photographs to browsers. The lightbox opens with the source already rendered in the selected hero or grid item, upgrades after its larger source loads, and maintains a bounded rolling cache of decoded responsive neighbors. Large editing sources remain in Git, while placeholder commerce and incomplete gallery focus management remain known limitations for later plans.

Release availability controls whether incomplete features appear in navigation and client routing; they do not authorize users, protect URLs, or remove code from production bundles. Changing `RELEASED_FEATURES` requires normal review and deployment. The all-features override is limited to Vite's `serve` command with the explicit `all-features` mode, so builds and previews retain the committed release state.
...
- **Current:** Zina Regular supplies display and brand typography at its designed Regular weight without synthesized bold. The official Zina OTF is self-hosted under its ITF Free Font License; do not convert, subset, rename, or otherwise modify the font file because that license prohibits derivative font software.
- Use the readable sans-serif stack for body copy. Inter Regular is used for the footer copyright and the approved supporting copy and links on Contact and Shop. Do not synthesize unsupported Zina weights.
...
The dark canvas is `#0e0e0e`. Zina Regular supplies all display and brand typography. The wordmark uses the muted navigation color `#BBB4A9` and aligns its lettering with the navigation-label line. Active navigation uses warm orange, its underline uses vivid violet, and inactive navigation remains muted while gaining the focus color on hover or keyboard focus. The body stack remains a readable sans serif, while approved supporting copy and links use Inter Regular. The 16rem footer is fixed behind the opaque application surface and revealed through an explicit footer-height flow spacer. Its signature remains cream and its only text is the 10px Inter Regular copyright. Reduced-motion users receive the complete static footer without the scroll-linked transform.

Motion for React is the selected general-purpose runtime for coordinated entrances and exits, page transitions, scroll reveals, scroll-linked transforms, bounded parallax, and justified layout animation. Plan 010 introduced its first narrow footer use; Plan 011 establishes strict lazy feature loading, shared reduced-motion policy, and the first approved landing effect. Plan 012 applies Motion only to element-scoped image transforms; CSS owns the sticky and bounded stacking layout. Plan 013 adds a short opacity/transform route transition around the router location; outgoing route content is inert and hidden from assistive technology, while the established CSS navigation indicator remains the chosen shared-layout treatment because Motion layout features add no demonstrated value. Touch, keyboard, upward, modal, and reduced-motion scrolling remain browser-owned. Ordinary downward desktop wheel distance is replayed 1:1 through one footer-owned animation-frame controller above the final viewport, then eased only within that viewport. This is a measured product exception rather than a general smooth-scroll system. The platform scrollbar remains visible with restrained CSS styling.
...
Use strict `LazyMotion` with slim `m` components and the smallest feature bundle that supports implemented behavior. The current `domAnimation` features ship with the application entry because route transitions must work on the first interaction after a cold load; this removes the separate feature request and slightly reduces total compressed JavaScript while adding about 10.6 kB gzip to the entry chunk. Measure bundle and runtime cost, and add layout or gesture features only when an accepted interaction requires them. Introduce another animation runtime only with a concrete, measured justification.
