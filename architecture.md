...
Use strict `LazyMotion` with slim `m` components and the smallest feature bundle that supports implemented behavior. The current `domAnimation` features load through a separate dynamic module after semantic content can render. Measure bundle and runtime cost, and add layout or gesture features only when an accepted interaction requires them. Introduce another animation runtime only with a concrete, measured justification.
