...
- Open enlarged media with the selected element's already-rendered source as an immediate preview, size the viewer stage from catalog dimensions, then replace the preview only after the larger responsive source decodes. Keep preview selection and preloading in the portfolio feature rather than the shared image component.
