...
- Open enlarged media with the selected element's already-rendered source as an immediate blurred preview, size the viewer stage from catalog dimensions, then replace the preview only after the larger responsive source decodes. Gate adjacent navigation during that short handoff so repeated input cannot expose undecoded frames. Keep preview selection and preloading in the portfolio feature rather than the shared image component.
