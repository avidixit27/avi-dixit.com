...
The dark canvas is `#0e0e0e`. Zina Regular supplies all display and brand typography. The wordmark uses the muted navigation color `#BBB4A9` and aligns its lettering with the navigation-label line. Active navigation uses the focus color `#FFE193`; inactive navigation remains muted and gains the focus color on hover or keyboard focus. The body stack remains a readable sans serif, while approved supporting copy and links use Inter Regular. The 16rem footer is fixed behind the scrolling application surface and uses a bounded scroll-linked parallax transform across viewport sizes while preserving native document scrolling. Its only text is the 10px Inter Regular copyright. Reduced-motion users receive the complete static footer without the scroll-linked transform.
...
Prefer strict `LazyMotion` with the slim `m` components and the smallest feature bundle that supports implemented behavior. Plan 010's narrow direct Motion footer import is an accepted first use with a measured bundle cost; Plan 011 owns the provider and lazy-loading audit. Load animation features after semantic content can render, measure bundle cost, and add layout or gesture features only when an accepted interaction requires them. Introduce another animation runtime only with a concrete, measured justification.
