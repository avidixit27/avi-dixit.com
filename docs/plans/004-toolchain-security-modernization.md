...
- Implementation merged through [PR #14](https://github.com/avidixit27/avi-dixit.com/pull/14). All frontend and security jobs passed on Node `22.22.2`: lint/format/types, unit tests, Cypress component tests, production build/E2E, dependency review, production audit, and CodeQL.
- Dependabot PRs #7, #8, and #9 remained open after the merge because they are routine major-version proposals, not unresolved vulnerability alerts. The update configuration now suppresses ESLint 10 until its React/accessibility peer graph is valid and keeps `@types/node` on the supported Node 22 major. GitHub vulnerability alerts and npm audits remain available even when a routine version proposal is ignored.
