...
Post-review correction: preserve the existing camel-case custom-color contract in Tailwind 4 (`accentWarm`, `accentCool`, and `accentVivid`). The initial kebab-case variables did not generate the established utility names used by navigation, lightbox, contact, and shop. A production build now confirms the expected `text-accentVivid`, `hover:bg-accentWarm`, `hover:text-accentWarm`, and `focus:border-accentWarm` rules. The follow-up coverage-planning ticket will define a meaningful automated regression guard for this category of configuration change.
