...
The optional [Ponytail Codex plugin](https://github.com/DietrichGebert/ponytail) reinforces this decision order. Install it with `codex plugin marketplace add DietrichGebert/ponytail` and `codex plugin add ponytail@ponytail`; review its lifecycle hooks, restart Codex, and start a new task to use it. These repository instructions and the approved plan remain authoritative for scope, safety, accessibility, and verification even when the plugin is active.
